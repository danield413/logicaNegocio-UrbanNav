...
export class AdministradorDeArchivosController {
  ...
  async CargarArchivoUsuario(
    ...
    const filePath = path.join(__dirname, ConfiguracionLogica.carpetaArchivosUsuarios);
    let res = await this.StoreFileToPath(
      filePath,
      ConfiguracionLogica.campoDeUsuario,
      request,
      response,
      ConfiguracionLogica.extensionesImagenes,
    );
    if (res) {
  ...
  private ObtenerArchivosPorTipo(tipo: number) {
    ...
    switch (tipo) {
      // amusement
      case 1:
        filePath = path.join(__dirname, ConfiguracionLogica.carpetaArchivosUsuarios);
        break;
      case 2:
        break;
      case 3:
